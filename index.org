#+TITLE: 민더스트리 모드 가이드
#+SUBTITLE: Modding documentation for Mindustry v104 (0cbd9a1fc)
:PREAMBLE:

#+AUTHOR: Sharlotte
#+EMAIL: aaa9810321@gmail.com
#+DATE: 2020-09-01
...
이슈, 제안, PR를 추천할 깃헙 주소: https://github.com/sharlotte-mobile/sharlotte-mobile.github.io
...
* Overview
  ...
  민더스트리 모드는 단순히 에셋(assests)의 디렉토리일 뿐이고, 정확히 무엇을 하고 싶은지, 어디까지 할 의향이 있는지에 따라 모딩 API를 활용하는 방법은 여러 가지가 있습니다.

  기존 바닐라 콘텐츠를 다시 게시하거나, 보다 단순한 Json API(이 위키의 핵심부분인)로 새로운 게임 콘텐츠를 만들 수 있으며, 사용자 지정 사운드를 추가하거나 기존 사운드를 다시 사용할 수 있습니다. 캠페인 모드에 지도를 추가하고, 사용자 정의 효과와 같은 특수 동작을 모드에 프로그래밍하는 스크립트(자바 스크립트, JavaScript)를 추가할 수 있습니다. (자바 스크립트는 (h)json과 달리 난이도가 매우 높아요)

  당신의 mod를 공유하는 것은 당신의 프로젝트 디렉토리(=모드)를 다른 사람에게 주는 것만큼 간단합니다; 모드는 또한 그 모드를 지원하는 플랫폼 또한 크로스 플렛폼입니다. 현실적으로 말해서 GitHub를 사용하고 싶을 것일텐데, GitHub: https://github.com/Anuken/ExampleMod의 예제 모드 저장소를 확인하는게 좋습니다.

  솔직히, 모드를 만드는데 있어서 정말 중요한 것은 텍스트 편집기를 가진 아무 컴퓨터. 즉, 매모장만 있으면 됩니다.
...
** Directory Structure
   ...
   * [[#modjson][~mod.json~]] (필수) 모드를 위한 메타데이터 파일,
   * ~content/*~디렉토리: 게임 [[Content][Content]] (콘텐츠),
   * ~maps/~ 디렉토리: [[Zone][Zone]] 맵,
   * ~bundles/~ 디렉토리: [[Bundles][Bundles]],
   * ~sounds/~ 디렉토리: [[Sound][Sound]] (소리)파일,
   * ~schematics/~ 디렉토리: [[Schematic][Schematic]] (설계도)파일,
   * ~scripts/~ 디렉토리: [[Scripts][Scripts]],
   * ~sprites-override/~ [[Sprites][Sprites]] (스프라이트)디렉토리: 인게임 콘텐츠를 재정의,
   * ~sprites/~ [[Sprites][Sprites]] (스프라이트)디렉토리: 당신의 모드 콘텐츠,

   모든 플렛폼들은 각자 다른 어플리케이션 데이터 디렉토리를 가지고 있는데, 이건 당신의 모드들이 배치될 경로:
   ...
   * Apple: =~/Library/Application Support/Mindustry/mods/=
   ...
   /파일 이름은 소영문자에 공백을 하이픈(-) 문장으로 사용해야 됩니다:/
   * 정상: ~my-custom-block.json~
   * 비정상: ~My Custom Block.json~

** Hjson

   민더스트리는 Json을 아는 많은 사람들을 위해 [[https://hjson.org/][Hjson]] 를 사용하는데, 이 [[https://hjson.org/][Hjson]] 는 [[https://en.wikipedia.org/wiki/JSON][Json]]이라는 매우 유명한 직렬화 언어의 상위집합입니다. -- 이건 json으로도 작동한다는 것을 의미하는데, 다음과 같은 예제에서 추가적인 정보를 얻을 수 있습니다:
   ...
   만약 당신이 여기 단어중 하나라도 모르신다면. --직렬화 언어는 프로그램의 정보를 인코딩하는 언어일 뿐이며, 인코딩은 자료형을 한 형태에서 다른 형태로, 그리고 이 경우 텍스트를 자바 데이터 구조로 변환하는 것을 의미합니다.
...
** ~mod.json~
   ...
   프로젝트 디렉터리의 루트에는 프로젝트의 기본 메타데이터를 정의하는 mod.json이 있어야 합니다. 또한 이 파일은 텍스트 편집기가 더 나은 구문 강조 표시를 선택하는 데 도움이 되도록 mod.hjson이라는 이름을 (선택적으로) 지정할 수 있습니다.

   #+BEGIN_SRC yaml
   name: 모드 이름
   displayName: 모드 [red]이름[]
   author: 당신의이름
   description: 멋진 모드의 설명!
   version: "1.0"
...
** Scripts
   ...
   * [[https://github.com/DeltaNedas/entity-lib][entity-lib]]

** GitHub

   Once you have a mod of some kind, you'll want to actually share it, and you may even want to work with other people on it, and to do that you can use [[https://github.com/][GitHub]]. If you don't know what Git (or GitHub) is at all, then you should look into [[https://desktop.github.com/][GitHub Desktop]], otherwise simply use your favorite command line tool or text editor plugin.
...
**** Mar 5
     :PROPERTIES:
     :UNNUMBERED: t
     :CUSTOM_ID: 50355b45d5eb5c09c19f8d7c6bba48c568a609a8
     :END:
     [ [[https://github.com/Anuken/Mindustry/commit/50355b45d5eb5c09c19f8d7c6bba48c568a609a8][commit]] Tech tree balance ]
...
**** Feb 11
     :PROPERTIES:
     :UNNUMBERED: t
     :CUSTOM_ID: fb302d49c7de104675c7c33c0e98e4f31e735bd7
     :END:
     [ [[https://github.com/Anuken/Mindustry/commit/fb302d49c7de104675c7c33c0e98e4f31e735bd7][commit]] improved battery brightness display ]
...
**** Jan 24
     :PROPERTIES:
     :UNNUMBERED: t
     :CUSTOM_ID: 923d576a5fb7553eb28fa7ebc8a9c16fd4fe6ceb
     :END:
     [ [[https://github.com/Anuken/Mindustry/commit/923d576a5fb7553eb28fa7ebc8a9c16fd4fe6ceb][commit]] fixed #1436 / fixed crawlers not exploding ]
...
**** Jan 23
     :PROPERTIES:
     :UNNUMBERED: t
     :CUSTOM_ID: 235142c8698f0d8cd9fc296a2ecabf430d7ea261
     :END:
     [ [[https://github.com/Anuken/Mindustry/commit/235142c8698f0d8cd9fc296a2ecabf430d7ea261][commit]] implemented #1093 ]
...
**** Jan 22
     :PROPERTIES:
     :UNNUMBERED: t
     :CUSTOM_ID: a4e820f90733148de4b75f5b60928e0523bf51b3
     :END:
     [ [[https://github.com/Anuken/Mindustry/commit/a4e820f90733148de4b75f5b60928e0523bf51b3][commit]] added default ore flags for modded ores ]
...
**** Jan 19
     :PROPERTIES:
     :UNNUMBERED: t
     :CUSTOM_ID: b1df52e0c83ae44f5bd70892fcda5ac2fa69e36c
     :END:
     [ [[https://github.com/Anuken/Mindustry/commit/b1df52e0c83ae44f5bd70892fcda5ac2fa69e36c][commit]] cleanup of scripts ]
...
**** Jan 14
     :PROPERTIES:
     :UNNUMBERED: t
     :CUSTOM_ID: eaa86023f9854b6e8d574cbfdd43ee32488b2de5
     :END:
     [ [[https://github.com/Anuken/Mindustry/commit/eaa86023f9854b6e8d574cbfdd43ee32488b2de5][commit]] visual tweaks ]
...
**** Jan 08
     :PROPERTIES:
     :UNNUMBERED: t
     :CUSTOM_ID: 406c11a14d31c9680b914c60cd1d8577ddd5fc7d
     :END:
     [ [[https://github.com/Anuken/Mindustry/commit/406c11a14d31c9680b914c60cd1d8577ddd5fc7d][commit]] make rebuildable a block attribute (#1338) ]
...
**** Jan 07
     :PROPERTIES:
     :UNNUMBERED: t
     :CUSTOM_ID: b0d65dcedb26014580632a9b0d8d597f0c7f8ccc
     :END:
     [ [[https://github.com/Anuken/Mindustry/commit/b0d65dcedb26014580632a9b0d8d597f0c7f8ccc][commit]] cleanup ]
...
**** Jan 04
     :PROPERTIES:
     :UNNUMBERED: t
     :CUSTOM_ID: 1dd0295c45e6ea6f0967f291b41ff3a58ab7202c
     :END:
     [ [[https://github.com/Anuken/Mindustry/commit/1dd0295c45e6ea6f0967f291b41ff3a58ab7202c][commit]] merge remote-tracking branch 'origin/master' ]
...
**** Jan 03
     :PROPERTIES:
     :UNNUMBERED: t
     :CUSTOM_ID: 62b2b25ee474fcf44aa86832c1373b38e16d703d
     :END:
     [ [[https://github.com/Anuken/Mindustry/pull/1313/commits/62b2b25ee474fcf44aa86832c1373b38e16d703d][commit]] use findAll to iterate through mod content ]
...
**** Dec 12
     :PROPERTIES:
     :UNNUMBERED: t
     :CUSTOM_ID: 2366d25de73ab85bffc3352ab1e74ba1dea68dfe
     :END:
     [ [[https://github.com/Anuken/Mindustry/commit/2366d25de73ab85bffc3352ab1e74ba1dea68dfe][commit]] add liquid void block ]
...
**** ThermalGenerator
     Extends [[PowerGenerator][PowerGenerator]] -- Generates power with the [[Attributes][attribute]] of a tile. Power production is ~powerProduction * attribute~, and ~attribute~ must be greater then ~0.01~.
